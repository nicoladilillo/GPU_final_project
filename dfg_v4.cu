 ...
 * 
 * Improve repetition and use variable for final comb, repetition and k_comb
 */
...
// #define TESTING_SCHEDULING
 ...
 __global__ void combination(const int n, int r, const int tot_comb, const operation_GPU_t *Operation_init, 
    ...
    uint8_t *final_best_combination, uint8_t *final_best_repetition, int *final_best_time, int *final_area_calculated)
 {
    ...
    if (idx < tot_comb) {
        ...
        int operation_covered[30];

        // lenght node_number
        node_GPU_t node[150];
        // variable used from scheduling node
        uint8_t state[150];
        uint8_t remain_time[150];
        uint8_t id_resource[150];
        uint8_t dependecies_level_satisfy[150];

        // lenght K_comb
        uint8_t repeat[30];
        uint8_t index = 0;

        uint8_t best_combination[30], best_repetition[30];
