 ...
 * 
 * Improve memory usages and fix scheduling algorithm
 */
...
#define TESTING
 ...
 int main(int argc, char const *argv[])
 {
    ...
    operation_GPU_t *Operation_GPU = (operation_GPU_t *)malloc(operation_number*sizeof(operation_GPU_t));
    for(i = 0; i < operation_number; i++)
    {   
        Operation_GPU[i].operation_id = Operation[i].operation_id;
        Operation_GPU[i].res_occurency = Operation[i].res_occurency;
        Operation_GPU[i].res = (resource_t *)malloc(sizeof(resource_t)*Operation[i].res_occurency);
        for (j = 0; j < Operation[i].res_occurency; j++)
            Operation_GPU[i].res[j] = Operation[i].res[j];
        Operation_GPU[i].index_next_node_occurency = Operation[i].index_next_node_occurency;
        Operation_GPU[i].index_next_node = (uint8_t *)malloc(sizeof(uint8_t)*Operation[i].index_next_node_occurency);
        for (j = 0; j < Operation[i].index_next_node_occurency; j++)
        {
            if(i == 0)
            {
            ...
            Operation_GPU[i].index_next_node[j] = Operation[i].index_next_node[j];
        }
    }
    ...
    for(i = 0; i < len_node; i++)
    {
        printf("%d) Node: %s(%d) - Operation: %s" , node_GPU[i].id_node, node[node_GPU[i].id_node].name, node_GPU[i].id_node, Operation[node_GPU[i].index_operation].name);
        if (node[i].dependecies_level != 0) {
            printf(" - Dependecies: ");
            if (node[i].dep1_index != EMPTY_INDEX)
    ...
    for(i = 0; i < operation_number; i++)
    {
        printf("For %s the node are: ", Operation[Operation_GPU[i].operation_id].name);
        for(j = 0; j < Operation[i].index_next_node_occurency; j++)
    ...
    uint8_t *best_final = (uint8_t *)malloc(sizeof(uint8_t)*(count+1));   
    uint8_t *best_final_repetition = (uint8_t *)malloc(sizeof(uint8_t)*count);
    int best_time = 0x7fffffff;
    int area_calculated = 0x7fffffff;
    ...
    for(i = 0; i < count && best_final[i] != EMPTY_INDEX; i++) 
    {
        for(j = 0; j < operation_number; j++) 
        {
