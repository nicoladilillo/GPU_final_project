 ...
 * performed the check inside the block to see the best result
 */
...
// #define TESTING_SCHEDULING
    ...
    uint8_t *final_best_repetition, int *final_best_time, int *final_area_calculated)
 {
    ...
    if (idx >= start_comb && idx < end_comb) {
        ...
        node = (node_GPU_t *) &(s[0]);
        memory_trace += (int) (node_number*sizeof(node_GPU_t));            
        Operation = (operation_GPU_t *) &(s[memory_trace]);
        memory_trace += (int) (operation_number*sizeof(operation_GPU_t));      
        ...
        // lenght k_comb
        uint8_t *final             = new uint8_t[k_comb];
        uint8_t *repeat            = new uint8_t[k_comb];
        uint8_t *operation_covered = new uint8_t[operation_number];
        ...
        {   
            ...
            if(idx == i)
            {
                printf("\t%4d) ", idx);
                for(j = 0; j < k_comb; j++) 
                    printf("%2d %2d   ", final[j], repeat[j]);
                // printf(" -- Memory is from %d to %6d \n",
        ...
        for(i = 0; i < operation_number; i++)
            for(j = 0; j < Operation[i].res_occurency; j++)
                resources[i*Operation[i].res_occurency+j] = Operation[i].res[j];
        #endif
        for(z = 0; z < k_comb; z++)
        {
            for(i = 0; i < operation_number; i++)
            {
                for(j = 0; j < Operation[i].res_occurency; j++)
                {
                    if (Operation[i].res[j].id == final[z])
                    {
                        ...
                        area                         += (resources[final[z]].area * repeat[z]);
                    }
                }
            }
        }
        ...
        if (area <= area_limit)
        {
            ...
            if(flag == 1) 
            {
                            ...
                            if (remain_time[j] == 1) 
                            {
                                ...
                                if(idx == 1035 && k_comb == 3) {
                                    printf("END node %d (op %d -- state %d) at time %d with resources %d\n", j, 
                                        node[j].index_operation, state[j], time+1, id_resource[j]);
                                }
                                #endif
                ...
                free(state);
                free(remain_time);
                free(dependecies_level_satisfy);
                free(id_resource);
            }
        }
        ...
        for (i = 0; i < k_comb; i++)
        {
            final_combination[threadIdx.x*k_comb + i] = final[i];
            final_repetition[threadIdx.x*k_comb  + i] = repeat[i];
        }
        ...
        {   
            ...
            for(j = 0; j < end_comb-start_comb; j++)
            {
                if (final_time[j] == -1)
                {
                    printf("idx: %d --> No combination for ", j+start_comb);
                    for(i = 0; i < k_comb; i++)
                        printf("%d  ", final_combination[j*k_comb + i]);
                } else {
                    printf("idx: %d - Best time: %d - area: %d\n", j+start_comb, final_time[j], final_area[j]);
                    for(i = 0; i < k_comb; i++)
                        printf("%d  ", final_combination[j*k_comb + i]);
                        printf("\n");
                    for(i = 0; i < k_comb; i++)
                    {                        
                        printf("\tid: %d - occurency: %d - area: %d - speed: %d\n ",
                            final_combination[j*k_comb + i], final_repetition[j*k_comb + i], 
                            resources[final_combination[j*k_comb + i]].area,
                            resources[final_combination[j*k_comb + i]].speed);
                    }
                }
                printf("\n");   
            }
 ...
 int main(int argc, char const *argv[])
 {
    ...
    for(i = 0; i < max_stream_number; i++)
    {
        ...
        cudaMalloc(&dev_final_best_combination[i], (resource_number+1)*sizeof(uint8_t));
        final_best_combination[i] = (uint8_t *)malloc((resource_number+1)*sizeof(uint8_t));
    ...
    for(k = operation_used; k <= resource_number; k++) {
        ...
        shared_memory_size = (int) (k*sizeof(uint8_t)*2 +
                                    sizeof(int)*2);
