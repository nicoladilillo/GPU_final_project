 ...
 * 
 * Run kernel on more blocks for combination greater than 1024. This is a first attemp,
 * doesn't work with more than 2 blocks
 */
...
// #define TESTING_SCHEDULING
 ...
 __global__ void combination(const int n, int r, const int tot_comb, const int start_comb,
    const int end_comb, int const shared_memory_size, int const shared_memory_size_offset, 
    ...
    uint8_t *final_best_repetition, int *final_best_time, int *final_area_calculated)
 {
    ...
    if (idx < end_comb) {
        ...
        int *final_time = (int *) &(s[(int) (node_number*sizeof(node_GPU_t)) + (operation_number*sizeof(operation_GPU_t))]);
        int *final_area = (int *) &(s[(int) sizeof(int) + (node_number*sizeof(node_GPU_t)) + (operation_number*sizeof(operation_GPU_t))]);
        ...
        int memory_trace = (int) (shared_memory_size_offset + threadIdx.x*shared_memory_size);
        ...
        #ifdef TESTING
        for(i = 0; i < end_comb-start_comb; i++)
        {   
            __syncthreads();
            if(threadIdx.x == i)
            {
        ...
        #ifdef TESTING
        for(j = 0; j < end_comb-start_comb; j++)
        {   
            __syncthreads();
            if(threadIdx.x == j)
            {
                if (best_time == 0x7fffffff)
                {
                    ...
                    {                        
                        printf("\tid: %d - occurency: %d - area: %d - speed: %d\n ",
                            best_combination[i], best_repetition[i], 
                            resources[best_combination[i]].area, resources[best_combination[i]].speed);
                    }
                }
                printf("\n");   
            }
        ...
        for(i = 0; i < end_comb-start_comb; i++)
        {
            ...
            if(threadIdx.x == i) 
            { 
                if (best_time > -1 && (*final_time > best_time
                    || (best_time == *final_time && *final_area > area_calculated)))
                {
                    *final_time = best_time;
                    *final_area = area_calculated;
                    for(j = 0; j < k_comb; j++) 
                    {
                        final_best_combination[j] = best_combination[j];
                        final_best_repetition[j] =  best_repetition[j];
                    }
                }
            }
        }
 ...
 int main(int argc, char const *argv[])
 {
    ...
    int offset_shared_memory_size = int (operation_number*sizeof(operation_GPU_t) + node_number*sizeof(node_GPU_t)) + 2*sizeof(int);
    ...
    for(k = operation_used; k <= resource_number; k++) {
        ...
        // Invoke kernel
        int threadsPerBlock_d = 256;
        int blocksPerGrid_d = tot_comb / threadsPerBlock_d + 1;
        printf("Number of total blocks are: %d\n", blocksPerGrid_d);

        int end_comb;
        int start_comb = 0;
        // Go among group of MAX_BLOCKS
        for(j = 0; j < blocksPerGrid_d; j++)
        {
            start_comb = j*threadsPerBlock_d;
            end_comb   = (j+1)*threadsPerBlock_d;
            if (end_comb > tot_comb)
            {
                end_comb = tot_comb;
                threadsPerBlock_d = tot_comb - j * threadsPerBlock_d;
            }
            tot_shared_memory = offset_shared_memory_size + (shared_memory_size*tot_comb);

            cudaMalloc(&dev_final_best_time[stream_number], sizeof(int));
            
            cudaMalloc(&dev_final_area_calculated[stream_number], sizeof(int));

            cudaMalloc(&dev_final_best_combination[stream_number], k*sizeof(uint8_t));
            final_best_combination[stream_number] = (uint8_t *)malloc(k*sizeof(uint8_t));

            cudaMalloc(&dev_final_best_repetition[stream_number], k*sizeof(uint8_t));
            final_best_repetition[stream_number] = (uint8_t *)malloc(k*sizeof(uint8_t));

            printf("Number of total combination witk k equal to %d are: %d\n", k, tot_comb);
            printf("\tShared memory Total is %d -- Piece of shared memory is %d -- Offset is %d\n",
               tot_shared_memory, shared_memory_size, offset_shared_memory_size);
            printf("\tNumber of block is %d -- start comb is %d -- end comb is %d -- thread are %d\n", 
                j, start_comb, end_comb, threadsPerBlock_d);
            // call kernel
            combination<<<1, threadsPerBlock_d, tot_shared_memory, streams[stream_number]>>>(
                resource_number, k, tot_comb, start_comb, end_comb, 
                shared_memory_size, offset_shared_memory_size, dev_Operation, operation_number, dev_node, 
                node_number, area_limit, resource_number,
            ...
            if (stream_number == max_stream_number || j == blocksPerGrid_d-1)
            {
                printf("Arrived to %d\n", stream_number);

                for(i = 0; i < stream_number; i++) 
                {   
                    printf("\tWaiting for stream %d\n", i);
                    cudaStreamSynchronize(streams[i]);
                    cudaMemcpy(&final_best_time[i],        dev_final_best_time[i],        sizeof(int),       cudaMemcpyDeviceToHost);
                    cudaMemcpy(&final_area_calculated[i],  dev_final_area_calculated[i],  sizeof(int),       cudaMemcpyDeviceToHost);
                    cudaMemcpy(final_best_combination[i], dev_final_best_combination[i], k*sizeof(uint8_t), cudaMemcpyDeviceToHost);
                    cudaMemcpy(final_best_repetition[i],  dev_final_best_repetition[i],  k*sizeof(uint8_t), cudaMemcpyDeviceToHost);
                    ...
                    cudaFree(dev_final_best_time[i]);
                    cudaFree(dev_final_area_calculated[i]);
            ...
            // printf("\tFINISH Number of block is %d -- start comb is %d -- end comb is %d\n", cuda_block, start_comb, end_comb);
        }
                
        //#ifdef TESTING
        printf("Best Combination: ");
        for(i = 0; i < k && best_final[i] != EMPTY_INDEX; i++)
            printf(" %2d %2d,", best_final[i], best_final_repetition[i]);
