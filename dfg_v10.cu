...
// #define TESTING_SCHEDULING
    ...
    uint8_t *final_best_repetition, int *final_best_time, int *final_area_calculated, const int best_time, const int area_calculated)
 {
    ...
    if (idx >= start_comb && idx < end_comb) {
        ...
        if (threadIdx.x == 0)
        {
            // TO_DO: include also the index node and resources
        ...
        for(i = 0; i < k_comb; i++)
        {
            ...
            while (Choose(a,b) > x)
                --a;
            x = x - Choose(a,b);
            final_combination[threadIdx.x*k_comb + i] = (uint8_t) a;
            b = b-1;
            // Calculate the new repetition
            final_repetition[threadIdx.x*k_comb + i] = (uint8_t) rep_id%max_repetition+1;
        ...
        for(z = 0; z < k_comb; z++)
        {
            for(i = 0; i < operation_number; i++)
            {
                for(j = 0; j < Operation[i].res_occurency; j++)
                {
                    if (Operation[i].res[j].id == final_combination[threadIdx.x*k_comb + z])
                    {
        ...
        if(threadIdx.x == 0)
        {
            ...
            if ((blockIdx.x*blockDim.x + i + start_comb) < tot_comb && final_time[best] > -1 && (best_time > final_time[best] 
                    || (best_time == final_time[best] && area_calculated > final_area[best])))
            {
                ...
                for(i = 0; i < k_comb; i++)
                {
                    final_best_combination[blockIdx.x*resources_number + i] = final_combination[best*k_comb + i];
                    final_best_repetition[blockIdx.x*resources_number  + i] = final_repetition[best*k_comb + i];
                }
            } else 
 ...
 int main(int argc, char const *argv[])
 {
    ...
    int final_best_time[MAX_BLOCKS], *dev_final_best_time; 
    int final_area_calculated[MAX_BLOCKS], *dev_final_area_calculated; 
    ...
    uint8_t *best_final_repetition = (uint8_t *)malloc(sizeof(uint8_t)*(resource_number+1));
    ...
    cudaMalloc(&dev_final_best_time,       sizeof(int)*MAX_BLOCKS);
    cudaMalloc(&dev_final_area_calculated, sizeof(int)*MAX_BLOCKS);

    // allocate with max number possible
    cudaMalloc(&dev_final_best_combination, MAX_BLOCKS*resource_number*sizeof(uint8_t));
    final_best_combination = (uint8_t *)malloc(MAX_BLOCKS*resource_number*sizeof(uint8_t));

    cudaMalloc(&dev_final_best_repetition, MAX_BLOCKS*resource_number*sizeof(uint8_t));
    final_best_repetition = (uint8_t *)malloc(MAX_BLOCKS*resource_number*sizeof(uint8_t));
    ...
    for(k = operation_used; k <= resource_number; k++) {
        ...
        while(end_comb != tot_comb)
        {
            ...
            //call kernel
            combination<<<block_d, threadsPerBlock_d, tot_shared_memory>>>(
                ...
                dev_final_best_combination, dev_final_best_repetition,
                dev_final_best_time, dev_final_area_calculated, best_time, area_calculated);
            ...
            cudaMemcpy(final_best_time,        dev_final_best_time,        MAX_BLOCKS*sizeof(int), cudaMemcpyDeviceToHost);
            cudaMemcpy(final_area_calculated,  dev_final_area_calculated,  MAX_BLOCKS*sizeof(int), cudaMemcpyDeviceToHost);
            ...
            for(i = 0; i < block_d; i++)
            {
                ...
                {
                    cudaMemcpy(best_final,            (void *) (dev_final_best_combination + i*resource_number), resource_number*sizeof(uint8_t), cudaMemcpyDeviceToHost);
                    cudaMemcpy(best_final_repetition, (void *) (dev_final_best_repetition + i*resource_number),  resource_number*sizeof(uint8_t), cudaMemcpyDeviceToHost);

                    best_final[k]   = EMPTY_INDEX;
