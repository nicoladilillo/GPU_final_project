 ...
 * In this version all repeated combination has calculated and
 * limitator is removed
 * 
 */
#include <stdio.h>
#include <malloc.h>
...
const int node_number, int *best_final, int *best_final_repetition, const int area_limit, int *best_time, int *area_calculated)
{
    ...
    } else 
    {
        for(i = 0; i < operation_number; i++)
        {
            ...
            if (Operation[i].covered != 1)
            {
                index = r;
                #ifdef TESTING
                fprintf(fp, "Not all operations covered!\n");
                #endif
            }
        }
    }
    ...
    while(index != r) {
        ...
        // take in consideration all repetition
        index = 0;
