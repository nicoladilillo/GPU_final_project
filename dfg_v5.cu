...
// #define TESTING_SCHEDULING
 ...
 __global__ void combination(const int n, int r, const int tot_comb, const int start_comb, const int end_comb, 
    const operation_GPU_t *Operation_init, const int operation_number, const node_GPU_t *node_init, const int node_number, const int area_limit,
    uint8_t *final_best_combination, uint8_t *final_best_repetition, int *final_best_time, int *final_area_calculated)
 {
    ...
    if (idx < end_comb) {
        ...
        operation_GPU_t Operation[15];
        resource_t resources[30];
        // variable used for operation covered
        int operation_covered[15];

        // lenght node_number
        node_GPU_t node[120];
        // variable used from scheduling node
        uint8_t state[120];
        uint8_t remain_time[120];
        uint8_t id_resource[120];
        uint8_t dependecies_level_satisfy[120];

        // lenght K_comb
        uint8_t repeat[17];
        uint8_t index = 0;

        uint8_t best_combination[17], best_repetition[17];
        ...
        //     printf("We are here 2 %d\n", idx);
        //     for(i = idx+1; i < 1024 && i < tot_comb; i++)
        //     {   
        ...
        //         {
        //             final_best_time[idx] = final_best_time[i];
        //             best_time = final_best_time[i];
        //             final_area_calculated[idx] = final_area_calculated[i];
        //             area_calculated = final_area_calculated[i];
        //             for(j = 0; j < k_comb; j++) 
        //             {
        //                 final_best_combination[idx] = final_best_combination[i*k_comb+j];
        //                 final_best_repetition[idx] = final_best_repetition[i*k_comb+j];
        //             }
        //         }
        //     }
        // }
 ...
 int main(int argc, char const *argv[])
 {
    ...
    for(k = operation_used; k <= resource_number; k++) {
        ...
        final_best_time = (int *)malloc(tot_comb*sizeof(int));
        
        cudaMalloc(&dev_final_area_calculated, tot_comb*sizeof(int));
        final_area_calculated = (int *)malloc(tot_comb*sizeof(int));

        cudaMalloc(&dev_final_best_combination, k*tot_comb*sizeof(uint8_t));
        final_best_combination = (uint8_t *)malloc(tot_comb*k*sizeof(uint8_t));

        cudaMalloc(&dev_final_best_repetition, k*tot_comb*sizeof(uint8_t));
        final_best_repetition = (uint8_t *)malloc(tot_comb*k*sizeof(uint8_t));
        ...
        int end_comb, start_comb;
        int cuda_block;
        // Go among group of MAX_BLOCKS
        for(j = 0; j < blocksPerGrid_d; j += 4)
        {
            start_comb = (j)*(threadsPerBlock_d);
            end_comb = (j+4)*(threadsPerBlock_d);
            cuda_block = 4;
            if (end_comb > tot_comb) {
                end_comb = tot_comb;
                cuda_block = blocksPerGrid_d-j;
            }

            // call kernel
            cudaStreamCreate(&streams[stream_number]);
            printf("\tNumber of block is %d -- start comb is %d -- end comb is %d\n", cuda_block, start_comb, end_comb);
            combination<<<blocksPerGrid_d, threadsPerBlock_d, 0, streams[stream_number]>>>(resource_number, k, tot_comb, start_comb, end_comb, dev_Operation, 
                operation_number, dev_node, len_node, area_limit, dev_final_best_combination, 
                dev_final_best_repetition, dev_final_best_time, dev_final_area_calculated);
            
            cudaStreamSynchronize(streams[stream_number++]);
            stream_number++;
            // printf("\tFINISH Number of block is %d -- start comb is %d -- end comb is %d\n", cuda_block, start_comb, end_comb);
        }

        cudaMemcpy(final_best_time,        dev_final_best_time,        tot_comb*sizeof(int),       cudaMemcpyDeviceToHost);
        cudaMemcpy(final_area_calculated,  dev_final_area_calculated,  tot_comb*sizeof(int),       cudaMemcpyDeviceToHost);
        cudaMemcpy(final_best_combination, dev_final_best_combination, tot_comb*k*sizeof(uint8_t), cudaMemcpyDeviceToHost);
        cudaMemcpy(final_best_repetition,  dev_final_best_repetition,  tot_comb*k*sizeof(uint8_t), cudaMemcpyDeviceToHost);
        ...
        for(i = 0; i < tot_comb; i++)
        {
                ...
                || (final_best_time[i]  == best_time && final_area_calculated[i]  < area_calculated)))
            {
                for(j = 0; j < k; j++)
                {
                    best_final[j]            = final_best_combination[i*k+j];
                    best_final_repetition[j] = final_best_repetition[i*k+j];
                }
                best_final[j]   = EMPTY_INDEX;
                best_time       = final_best_time[i];
                area_calculated = final_area_calculated[i];
            }
        }
